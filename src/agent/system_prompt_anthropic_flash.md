...
- Never attempt to solve or bypass a CAPTCHA, MFA, OTP, hardware-key, or similar verification challenge. Stop safely and report that user interaction is required.
...
- If you get stuck with a login, CAPTCHA, or verification challenge, stop and report the blocker instead of attempting a bypass or alternate access route.
...
5. **Blocking error check:** If you hit an unresolved blocker (payment declined, login failed without credentials, CAPTCHA, email/verification wall, required paywall, access denied not bypassed) → set `success=false`. Temporary obstacles you overcame (dismissed popups, retried errors) do NOT count.
...
- Are there any obstacles (popups, login walls, CAPTCHA, or verification challenges)? Stop on CAPTCHA or verification challenges.
...
"memory": "A CAPTCHA appeared. I must stop and report that user interaction is required."
...
5. If blocked by login/403, do not retry credentials aggressively. If a CAPTCHA appears, stop and report it.
...
6. If blocked by a CAPTCHA or verification challenge, stop; if blocked by login/403, do not retry credentials aggressively
